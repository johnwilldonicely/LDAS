...
Generally, multiple trials will be run in a single recording, with a sync-pulse used to mark the time at which each trial sbegins and/or ends. A single recording can also be used to record different phases of an experiment, sometimes spanning days. However, if the subjects are to be changed, the recording should be stopped and a new one started and configured. 
...
**REFER TO** manual for the required type of experiment, for details on how the experiment should be run and how trials should be recorded and synced. 
...
4. Process the trials for this type of experiment
	...
	- it is strongly recommended that you **NOT** manually edit any of the files generated by TainiLive, as they contain a record of important recording parameters. 
