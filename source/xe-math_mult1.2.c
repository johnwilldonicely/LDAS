...
int main(int argc, char *argv[]) {
	...
	******************************************************************************/
	if(argc<2) 	{
		...
		fprintf(stderr,"%s\n",TITLE_STRING);
		fprintf(stderr,"----------------------------------------------------------------------\n");
		fprintf(stderr,"Multiply a column of data by a number\n");
		...
		fprintf(stderr,"	-v value to multiply this column by [%g]\n",setvalue);
		fprintf(stderr,"	-long: assume input and value are long-integers (0=NO 1=YES) [%d]\n",setlong);
		fprintf(stderr,"Examples:\n",setvalue);
		fprintf(stderr,"	cat datafile.txt | %s stdin -cy 1 -v 0.001\n",thisprog);
		fprintf(stderr,"	%s datafile.txt -cy 2 -v 100\n",thisprog);
		fprintf(stderr,"----------------------------------------------------------------------\n");
		fprintf(stderr,"\n");
		exit(0);
	}
	...
	for(ii=2;ii<argc;ii++) {
		if( *(argv[ii]+0) == '-') {
			...
			else if(strcmp(argv[ii],"-v")==0)  valarg=++ii;
	...
	/* OPEN FILE OR STDIN TO READ */
	if(strcmp(infile,"stdin")==0) fpin=stdin;
	else if((fpin=fopen(infile,"r"))==0) {fprintf(stderr,"\n--- Error[%s]: file \"%s\" not found\n\n",thisprog,infile);exit(1);}

	/* OPTION-1: CALCULATION USING DOUBLE-PRECISION FLOATS */
	if(setlong==0) {
		if(valarg!=-1) setvalue=atof(argv[valarg]);
		while(fgets(line,MAXLINELEN,fpin)!=NULL) {
			...
			for(col=1;(pcol=strtok(pline," ,\t\n"))!=NULL;col++) {
				...
				if(col==dvcol||dvcol==-1) {
					...
					printf("%f",(aa*setvalue));
				}
				else printf("%s",pcol);
			}
			printf("\n");
	}}
	/* OPTION-2: CALCULATION USING LONG-INTEGERS */
	else if(setlong==1) {
		if(valarg!=-1) setvaluelong=atol(argv[valarg]);
		while(fgets(line,MAXLINELEN,fpin)!=NULL) {
			if(line[0]=='#') continue;
			pline=line;
			for(col=1;(pcol=strtok(pline," ,\t\n"))!=NULL;col++) {
				...
				if(col>1)printf("\t",pcol);
				if(col==dvcol||dvcol==-1) {
					if(sscanf(pcol,"%ld",&jj)!=1) {printf("%s",pcol);continue;}
					printf("%ld",(jj*setvaluelong));
				}
				else printf("%s",pcol);
			}
			printf("\n");
