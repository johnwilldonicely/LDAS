...
SAMPLE CALL:
	...
	while(!feof(fpin)) {
		...
		else for(ii=0;ii<parameters[3];ii++) printf("%d\n",data[ii]);
	}
...
int xf_readbin1_i(FILE *fpin,void *buffer, int *data1, off_t *parameters, char *message) {
...
//TEST:for(ii=0;ii<3;ii++) fprintf(stderr,"parameters[%ld]=%ld\n",ii,parameters[ii]);
	...
	/************************************************************
	CONVERT NUMBERS ACTUALLY READ TO INT
	************************************************************/
	if(datatype==0) {
		unsigned char *pdata = buffer;
		for(ii=0;ii<nread;ii++) data1[ii]=(int)pdata[ii];
	}
	else if(datatype==1) {
		char *pdata = buffer;
		for(ii=0;ii<nread;ii++) data1[ii]=(int)pdata[ii];
