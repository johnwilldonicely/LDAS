...
int main (int argc, char *argv[]) {
	...
	char header[256],tempheader[256],*pid=NULL;
	...
	/* parse match-id list */
	if(setid!=NULL) {
		iid= xf_lineparse2(setid,",",&nid);
		if(iid==NULL) {{fprintf(stderr,"\n--- Error[%s]: memory allocation error\n\n",thisprog);exit(1);}}
	}
	...
	while(1) {
		...
		if(setid!=NULL) {
			...
			for(ii=0;ii<nid;ii++) {
				/* create temp id-pointer and the long and double forms of the id */
				pid= (setid+iid[ii]);
				idlong= atol(pid);
				iddouble= atof(pid);
//??? match doesn't seem to be working ; fprintf(stderr,"pid=%s\n",pid);
				/* reject if the id-column doesn't partial match the set id */
				if(setmatch==1 && strstr(pcol,pid)==NULL) continue;
				/* reject if the id-column doesn't exact match the set id */
				if(setmatch==2 && strcmp(pcol,pid)!=0) continue;
