#define thisprog "xe-ldas5-spectproc1"
...
double *xf_filter_FIRcoef1(int ftaps, int fpass, double fomegac, double fbw, char *filtwintype, double fbeta, char *message);
...
int main (int argc, char *argv[]) {
	...
	int setout=0, setverb=0;
	double setdiv=2.0,setffreq=0.0125;
	...
	if(argc<2) {
		...
		fprintf(stderr,"- adaptive smoothing (more smoothing at higher frequencies)\n");
		fprintf(stderr,"- FIR filtering to flatten the spectrum and reduce -ive AUC results\n");
		fprintf(stderr,"- calculates AUC for peaks, relative to line connecting valleys\n");
		fprintf(stderr,"USAGE: \n");
		fprintf(stderr,"\t%s [infile]\n",thisprog);
		fprintf(stderr,"	[infile]: file or stdin in format [freq] [power]\n");
		fprintf(stderr,"VALID OPTIONS: defaults in []\n");
		fprintf(stderr,"	-bands: CSV band-triplets (start,mid,stop) [unset=auto]\n");
		fprintf(stderr,"	-out: stdout output (0= file-only, 1=spectrum, 2=bands) [%d]\n",setout);
		fprintf(stderr,"	-verb: verbose output (0=NO 1=YES) [%d]\n",setverb);
		fprintf(stderr,"...smoothing options...\n");
		fprintf(stderr,"	-div: divisor in width= freq/div  (0=SKIP) [%g]\n",setdiv);
		fprintf(stderr,"...filter options (high-pass FIR, 101 taps)...\n");
		fprintf(stderr,"	-filt: HP filter frequency (0=SKIP) [%g]\n",setffreq);
		fprintf(stderr,"OUTPUT:\n");
		fprintf(stderr,"	%s: the modified spectrum\n",out2);
		fprintf(stderr,"	%s: stats for each detected band\n",out1);
	...
	*******************************************************************************/
	...
	- be careful if number of samples is less than default number of taps!
	*******************************************************************************/
	if(setffreq>0.0) {
		if(ftaps>(nn/2)) ftaps=nn/2;
		if(setverb==1) fprintf(stderr,"\t- filtering: HP=%g\n",setffreq);
		fomegac= 2.0*(setffreq/spectfreq) ;
		aa= 2.0*(3.0/spectfreq);
		coefs= xf_filter_FIRcoef1(ftaps,fpass,fomegac,aa,"kaiser",fbeta,message);
		if(coefs==NULL) { fprintf(stderr,"\n*** %s/%s\n\n",thisprog,message); exit(1); }
		x= xf_filter_FIRapply1_f(amp2,nn,coefs,ftaps,1,message);
	...
	*******************************************************************************/
	...
	/*******************************************************************************
	CALCULATE THE LOCAL PEAK-AMPLITUDE AND AUC
	*******************************************************************************/
	...
	for(ii=0;ii<nbands;ii++) {
		...
		if(z<0) { fprintf(stderr,"\n*** %s/%s\n\n",thisprog,message); exit(1); }
		auc1a= result_d[0]; auc1b= result_d[1]; auc1c= result_d[2];
