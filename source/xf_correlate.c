...
int xf_correlate(float *x, float *y, int N, int invalid, float *result) {
	...
	double
		...
		SSx=0.0,SSy=0.0,SDx=0.0,SDy=0.0,SSreg=0.0,SSres=0.0,SPxy=0.0,
		r=0.0,r2=0.0,r2adj=0.0,SE=0.0,PIP=0.0,F=0.0;
...
if(r>=.999999||r<=-.999999) {F=999999.0 ; prob=0.0; }
else if(F>0&&dfa>0&&dfb>0&&SDx!=0) prob = xf_prob_F(F,dfa,dfb);
else {r=r2=F=prob=-1.0;N=-1;} // this may happen if, for example, there is no variance in the predictor
