...
int xf_interp4_s(short *data, long ndata, short invalid, int setfill, long *result) {
	...
	/* initialize number of interpolated values */
	...
	for(ii=ii;ii<ndata;ii++) {
		...
		if(data[ii]!=invalid) { jj=ii; continue; }
			...
			if(ii<ndata) {
				count+=((ii-jj)-1);
				/* caluculate the amount to increment data during interpolation */
				step= (double)(data[ii]-data[jj])/(double)(ii-jj);
				/* interpolate, setfill the values between the invalid bookends (jj to ii) */
				for(kk=jj+1;kk<ii;kk++) data[kk]= (short)(data[kk-1]+step);
				/* make sure jj is reset to the current point, because at the end of this loop ii++ */
				jj=ii;
			}
		}
	}
	/* setfill the end of the array with the last valid datum */
