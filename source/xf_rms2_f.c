...
int xf_rms2_f(float *input, float *output, size_t nn, size_t nwin1, char *message) {
	...
	char *thisfunc="xf_rms2_f\0";
	size_t ii,jj,kk,mm,nwin2,start,stop,oddnwin1=0;
	double sum,s1,s2,rms,scale,dmm;
	...
	output[start]=(float)sqrt(sum/dmm);
	...
	for(ii=start;ii<stop;ii++) {
		...
		if(sum>0.0) output[ii]=(float)sqrt(sum/++dmm);
		else output[ii]=(float)0.0;
	}
	...
	for(ii=start;ii<stop;ii++) {
		...
		sum+=s2*s2; /* add the number at the leading edge of the window */
		if(sum>0.0) output[ii]=(float)sqrt(sum*scale);
		else output[ii]=(float)0.0;
	}
