...
int xf_readbin1_d(FILE *fpin,void *buffer, double *data1, off_t *parameters, char *message) {
...
//TEST:for(ii=0;ii<3;ii++) fprintf(stderr,"parameters[%ld]=%ld\n",ii,parameters[ii]);
	...
	/************************************************************
	CONVERT NUMBERS ACTUALLY READ TO DOUBLE
	************************************************************/
	if(datatype==0) {
		unsigned char *pdata = buffer;
		for(ii=0;ii<nread;ii++) data1[ii]=(double)pdata[ii];
	}
	else if(datatype==1) {
		char *pdata = buffer;
		for(ii=0;ii<nread;ii++) data1[ii]=(double)pdata[ii];
