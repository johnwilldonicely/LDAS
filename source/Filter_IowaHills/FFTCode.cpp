...
void FFT(double *InputR, double *InputI, int N, TTransFormType Type)
{
 ...
 {
  fprintf(stderr,"Invalid FFT type or size.");
  return;
 }
 ...
 if(BufferR == NULL  || BufferI == NULL  ||
  ...
  {
   fprintf(stderr,"FFT Memory Allocation Error");
   return;
  }
...
void DFT(double *InputR, double *InputI, int N, int Type)
{
 ...
 if(SumR == NULL || SumI == NULL ||
  ...
  {
   fprintf(stderr,"Incorrect DFT Type or unable to allocate memory");
   return;
  }
...
void RealSigDFT(double *Samples, double *OutputR, double *OutputI, int N)
{
 ...
 if(TwiddleReal == NULL || TwiddleImag == NULL)
  {
   fprintf(stderr,"Failed to allocate memory in FIRFreqResponse");
   return;
  }
