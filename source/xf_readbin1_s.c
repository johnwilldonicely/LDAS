...
int xf_readbin1_s(FILE *fpin,void *buffer, short *data1, off_t *parameters, char *message) {
...
//TEST:for(ii=0;ii<3;ii++) fprintf(stderr,"parameters[%d]=%ld\n",ii,parameters[ii]);


	/************************************************************
	CONVERT NUMBERS ACTUALLY READ TO SHORT
	************************************************************/
	if(datatype==0) {
		unsigned char *pdata = buffer;
		for(ii=0;ii<nread;ii++) data1[ii]=(short)pdata[ii];
	}
	else if(datatype==1) {
		char *pdata = buffer;
		for(ii=0;ii<nread;ii++) data1[ii]=(short)pdata[ii];
