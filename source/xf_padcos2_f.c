...
float *xf_padcos2_f(float *data, size_t nn, size_t npad, int type, char *message) {

	char *thisfunc="xf_padcos2_f\0";
	size_t ii,jj,kk,index,n2;
	float aa;
...
//TEST: fprintf(stderr,"npad=%d\n",npad);
	...
	/********************************************************************************
	If padding padding the beginning of data, copy the data forward, last element first
	********************************************************************************/
	if(type==1||type==3) {
	...
	********************************************************************************/
	if(type==2) {
		...
		ii=nn; jj=ii-1; while(ii<n2) data[ii++] = data[jj--] *  0.5*(1.0-cosf((float)(n2-ii)*aa));
	}
	/********************************************************************************
	...
	********************************************************************************/
	if(type==3) {
		...
		ii=npad; jj=npad; while(ii-->0) data[ii]= data[jj++] * 0.5*(1.0-cosf((float)(ii)*aa));
		/* pad end with reversed & tapered copy of the old end of the array */
		ii=nn+npad; jj=ii-1; while(ii<n2) data[ii++] = data[jj--] *  0.5*(1.0-cosf((float)(n2-ii)*aa));
