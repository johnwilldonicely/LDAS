...
float *xf_bin_simple_f(float *data1, long n1, long binsize, int type, long *nbins, char *message) {
	...
	char *thisfunc="xf_bin_simple_f\0",message2[256];
	long ii=0,jj,kk,mm,nsums=0,binmax=0,bin=0;
	float *result=NULL,*pdata=NULL;
	double aa,bb,cc,sum=0.0;
	...
	for(ii=0;ii<mm;ii+=binsize) {
		...
		if(type==1) { // bins contain sum
			...
			if(nsums==0) result[bin]= NAN;
			else result[bin]= (float)(sum/(double)nsums);
		}
		else { // bins contain median - for this, use the percentile function
			result[bin]= xf_percentile2_f(pdata,binsize,50.0,message2);
		}
		bin++;
	}
