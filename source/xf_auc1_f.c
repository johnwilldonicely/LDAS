...
int xf_auc1_f(float *curvey, long nn, double interval, int ref, double *result ,char *message) {
	...
	aucpos=aucneg=0.0;
	for(ii=1;ii<nn;ii++) {
		x1=(double)(ii-1)*interval; // previous value of x
		y1=(double)curvey[ii-1]; // previous value of y
		x2=(double)ii*interval;   // current value of x
		y2=(double)curvey[ii];   // current value of y
