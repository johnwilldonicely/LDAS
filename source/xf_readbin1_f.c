...
int xf_readbin1_f(FILE *fpin,void *buffer, float *data1, off_t *parameters, char *message) {
...
//TEST:for(ii=0;ii<3;ii++) fprintf(stderr,"parameters[%ld]=%ld\n",ii,parameters[ii]);
	...
	/************************************************************
	CONVERT NUMBERS ACTUALLY READ TO FLOAT
	************************************************************/
	if(datatype==0) {
		unsigned char *pdata = buffer;
		for(ii=0;ii<nread;ii++) data1[ii]=(float)pdata[ii];
	}
	else if(datatype==1) {
		char *pdata = buffer;
		for(ii=0;ii<nread;ii++) data1[ii]=(float)pdata[ii];
